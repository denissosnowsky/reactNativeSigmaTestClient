...
export const authSignInThunk =
  (user: Omit<UserDTO, 'name' | 'photo'>): ThunkAction<void, AppState, void, AnyAction> =>
  async (dispatch: ThunkDispatch<AppState, void, AnyAction>): Promise<void> => {
    try {
      dispatch(authActions.authUserRequested());

      const userData = await apiService.post<UserDAO>('/auth/signin', {
        ...user,
      });
